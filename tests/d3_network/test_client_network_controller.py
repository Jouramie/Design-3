...
def test_when_pair_with_host_then_connect(socket):
    port = 1234
...
@patch('src.d3_network.client_network_controller.socket')
def test_given_paired_network_controller_when_wait_start_command_then_decode_received_communication(socket):
    encoder = MagicMock()
...
@patch('src.d3_network.client_network_controller.socket')
def test_given_waiting_start_command_when_receive_command_then_callback(socket):
    callback = MagicMock()
    encoder = MagicMock()
...
@patch('src.d3_network.client_network_controller.socket')
def test_given_waiting_start_command_when_receive_other_command_then_do_not_callback(socket):
    callback = MagicMock()
    encoder = MagicMock()
