...
class App(QApplication):
    def __init__(self, network, camera, table_camera_config, logger, config: dict):
        super(App, self).__init__([''])
        self.main_model = StationModel()
        self.main_controller = StationController(self.main_model, network, camera, table_camera_config, logger, config)
