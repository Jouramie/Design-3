...
class StationController(object):
    ...
    def start_robot(self):
        ...
        self.__network.send_start_command()
    ...
    def interactive_testing(self):
        while True:
            ...
            if command[0] == 'ir':
                self.__network.ask_infrared_signal()
                self.__check_infrared_signal()
            elif command[0] == 'grab':
                self.__network.send_grab_cube_command()
            elif command[0] == 'drop':
                self.__network.send_drop_cube_command()
            elif command[0] == 'led':
                self.__network.send_end_of_task_signal()
            elif command[0] == 'f':
    ...
    def __check_infrared_signal(self) -> int:
        try:
            return self.__network.check_infrared_signal()
    ...
    def __is_correctly_oriented(self):
        if self._model.wall_of_next_cube == "middle":
            if -5 < self._model.robot.orientation < 5:
                return True
            else:
                return False
        elif self._model.wall_of_next_cube == "up":
            if 85 < self._model.robot.orientation < 95:
                return True
            else:
                return False
        elif self._model.wall_of_next_cube == "down":
            if 265 < self._model.robot.orientation < 275:
                return True
            else:
                return False
        else:
